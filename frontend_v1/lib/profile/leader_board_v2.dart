...
class _MainLeaderboardViewState extends State<MainLeaderboardView> {
  ...
  void initState() {
    ...
    resort(_leaderboardUsers); // Sort the list here
    _leaderboardUsers = _leaderboardUsers.toList();
    if (_leaderboardUsers.length < 3) {
      while (_leaderboardUsers.length < 3) {
        _leaderboardUsers.add({});
      }
    }

    assignCompletedTasksToUsers(widget.tasks, _leaderboardUsers);
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                                ...
                                CupertinoActionSheetAction(
                                  onPressed: () {
                                    ...
                                    switch (sortOption) {
                                      case 'Coins (default)':
                                        _changeSort('coins');
                                        AppLogger.info("sort by coins");
                                      case 'Level':
                                        _changeSort('level');
                                        AppLogger.info("sort by level");
                                      case 'Tasks':
                                        _changeSort('completedTasks');
                                        AppLogger.info("sort by tasks");
                                      default:
                                        _changeSort('deadline');
                                        AppLogger.info("sort by deadline");
                                    }
                                  },
              ...
              ChartLeaderboard(leaderboardUsers: _leaderboardUsers),
              WeeklyInfo(
...
class ChartLeaderboard extends StatefulWidget {
  ...
  const ChartLeaderboard({super.key, required this.leaderboardUsers});

  final List<Map<String, dynamic>> leaderboardUsers;

  @override
  State<ChartLeaderboard> createState() => _ChartLeaderboardState();
}
...
class _ChartLeaderboardState extends State<ChartLeaderboard> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Color> colorPrimaryList = widget.leaderboardUsers
        .take(3)
    ...
    final List<Color> colorSecondaryList = widget.leaderboardUsers
        .take(3)
        .where((user) => user['username'] != null)
        .map((user) {
    ...
    return SizedBox(
      ...
      child: Row(
        ...
        children: widget.leaderboardUsers
            .take(3)
            .toList()
            .asMap()
            .entries
            .map((entry) {
          final index = widget.leaderboardUsers[entry.key]['username'] != null
              ? entry.key
              : null;
...
class WeeklyInfo extends StatefulWidget {
  ...
  const WeeklyInfo({super.key, required this.leaderboardusers});
...
class _WeeklyInfoState extends State<WeeklyInfo> {
  ...
  Widget build(BuildContext context) {
    ...
    AppLogger.info(widget.leaderboardusers);
    return SizedBox(
      width: width,
      height: height * 0.2,
